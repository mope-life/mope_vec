...
namespace mope
{
    ...
    {
        template <typename T>
        struct is_field_helper : public std::conditional_t<
    ...
    template <typename T>
    struct is_field : public detail::is_field_helper<std::remove_cv_t<T>>
    {};
    ...
    namespace detail
    {
        ...
        \*====================================================================*/
        template <std::size_t N, typename T, template <std::size_t, typename> class Vec>
        struct _base
        {
        protected:
            T elements[N]{};
        ...
        struct _vec : public _base<N, T, vec>
        {
            ...
            template <typename S>
            constexpr auto hadamard_product(const _vec<N, S>& v) const
            {
                vec<N, std::common_type_t<T, S>> res;
                for (std::size_t i = 0; i < N; ++i) {
                    res[i] = (*this)[i] * v[i];
            ...
            template <typename S>
            constexpr auto hadamard_division(const _vec<N, S>& v) const
            {
                vec<N, std::common_type_t<T, S>> res;
                for (std::size_t i = 0; i < N; ++i) {
                    res[i] = (*this)[i] / v[i];
            ...
            constexpr auto hadamard_inverse() const
            {
                vec<N, T> res;
                for (std::size_t i = 0; i < N; ++i) {
                    res[i] = 1 / (*this)[i];
        ...
        struct _mat : public _base<N, vec<M, T>, mat>
        {
            ...
            constexpr _mat(const std::initializer_list<T>& L)
            {
                ...
                auto remaining = static_cast<std::size_t>(
                    std::distance(iter, L.end())
                );
                for (std::size_t idx = 0; remaining > 0; ++idx) {
            ...
            template <std::size_t P, typename S>
            ...
            static row_op exchange(std::size_t row_1, std::size_t row_2)
            {
            ...
            static row_op scale(std::size_t row, T scalar)
            {
                return [row, scalar](Col& a) { a[row] *= scalar; };
            }

            static row_op add_row(
                std::size_t this_row, std::size_t other_row, T scalar
            )
            {
            ...
            constexpr vec<N, std::size_t> LUPdecompose()
            {
                static_assert(
                    is_field_v<T> && M == N,
                ...
                vec<N, std::size_t> pivots;
                for (std::size_t i = 0; i < N; ++i) {
                    pivots[i] = i;
                }

                auto partial_pivot =
                    [&row_ops, &pivots](Col& a_i, std::size_t pivot_idx) {
                        for (std::size_t row = pivot_idx + 1; row < N; ++row) {
                            if (a_i[row] != 0) {
                                row_ops.push_back(exchange(pivot_idx, row));
                                row_ops.back()(a_i);
                                std::size_t tmp = pivots[row];
                ...
                auto add_rows = [&row_ops](Col& a_i, std::size_t pivot_idx) {
            ...
            {
                // inverse matrices probably don't make sense unless our field
                // has inverses. specialize is_field if you disagree.
                static_assert(
                    is_field_v<T> && M == N,
                    "This method only makes sense on square matrices"
                    "over fields."
                ...
                for (std::size_t col = 0; col < N; ++col) {
                    Col ycol{};
                    ycol[pivots[col]] = 1;
                    for (std::size_t row = pivots[col] + 1; row < N; ++row) {
                        T sum = 0;
                        for (std::size_t i = pivots[col]; i < row; ++i) {
            ...
            {
                static_assert(
                    M == N, "Identity generally only exists for square matrices."
                );
                mat<N, vec<N, T>> res{};
                for (std::size_t i = 0; i < N; ++i) {
    ...
    // mat<2, vec<2, int>>
    // absence of other partial specializations prevents things like:
    // mat<2, int> m;
